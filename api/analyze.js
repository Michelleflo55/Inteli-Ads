...
export default async function handler(req, res) {
  ...
  const curiosityFrameworks = 'I am so pissed no one told me about PRODUCT | I did X and this is what happened | What you need to know before X | Why popular belief is BS | This PRODUCT is so good it should be illegal | I probably should not be showing you this | Most PERSONA have no clue this exists | Here is what they dont tell you about TOPIC | Dont do X until you watch this | I wish I tried this sooner | If nobody else is going to tell you I will';
  const personaFrameworks = 'PERSONA are losing it over PRODUCT | PERSONA are raving about PRODUCT | Why PERSONA is switching to PRODUCT | This is what PAIN POINT looks like for PERSONA | The best PRODUCT for PERSONA';

  const systemPrompt = [
    'You are a DTC ad strategist. Analyze brand and competitor data. Use real consumer language. Be specific. Never vague.',
    '',
    'PERSONA RULES: Write personas that match the actual target customer for this brand. BANNED: journey, empowered, confident, thriving, balance, self-care, 2pm slump, overwhelmed, juggling, seamlessly, unlock, transform, elevate, lifestyle.',
    '',
    'GENDER RULE: Personas must match the actual target customer gender. A mens health brand gets male personas with why he buys. A womens brand gets female personas with why she buys. A unisex brand gets mixed. Infer from brand and product category.',
    '',
    'HOOK RULE: Never mention competitor names in any hook. Hooks stand alone on brand merits only.',
    '',
  ...
  const schema = [
    ...
    '  "personas": [',
    '    {"type":"proven","identity":"4-6 words who they are","description":"2 sentences their daily life","why_they_buy":"specific moment they try this, correct gender pronoun","hook":"first ad line max 12 words","angle":"how product fits their life","redditQuote":"real consumer quote"},',
    ...
    '    "topic": ["15-20 topic words, occasions, body types, life stages relevant to this brand"]',
    '  }',
    '}'
  ].join('\n');

  const userMessage = 'Brand: ' + brand + '\n\nData:\n' + context + '\n\nNegative frameworks: ' + negativeFrameworks + '\nCuriosity frameworks: ' + curiosityFrameworks + '\nPersona frameworks: ' + personaFrameworks + '\n\nReturn this exact JSON with real data:\n' + schema;
